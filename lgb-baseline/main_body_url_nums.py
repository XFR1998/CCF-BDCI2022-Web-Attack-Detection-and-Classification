...
print('-------------------提取ti-idf特征 end...------------------------')




# print('-------------------提取body的len特征 start...------------------------')
# df['body_len'] = df['body'].apply(lambda x: len(list(x)))
# print('-------------------提取body的len特征 end...------------------------')



print('-------------------提取body的url_nums特征 start...------------------------')
...
def extract_body_url_nums(x):
    ...
    try:
        ...
        # print(x)
        return -1
df['body_url_nums'] = df['body'].apply(lambda x: extract_body_url_nums(x))
print('-------------------提取body的url_nums特征 end...------------------------')


# print('-------------------提取url的各项解析统计特征 start...------------------------')
# from urllib.parse import urlparse
# def extract_url_info(x):
...
#
#
# scheme = df['url'].apply(lambda x: urlparse(x).scheme)
# le = LabelEncoder()
# df['scheme'] = le.fit_transform(scheme)
# df['scheme_len'] = df['url'].apply(lambda x: len(urlparse(x).scheme))
#
#
# netloc = df['url'].apply(lambda x: urlparse(x).netloc)
# le = LabelEncoder()
# df['netloc'] = le.fit_transform(netloc)
# df['netloc_len'] = df['url'].apply(lambda x: len(urlparse(x).netloc))
#
#
# path = df['url'].apply(lambda x: urlparse(x).path)
# le = LabelEncoder()
# df['path'] = le.fit_transform(path)
# df['path_len'] = df['url'].apply(lambda x: len(urlparse(x).path))
#
#
# parameters = df['url'].apply(lambda x: urlparse(x).params)
# le = LabelEncoder()
# df['parameters'] = le.fit_transform(parameters)
# df['parameters_len'] = df['url'].apply(lambda x: len(urlparse(x).params))
#
#
# query = df['url'].apply(lambda x: urlparse(x).query)
# le = LabelEncoder()
# df['query'] = le.fit_transform(query)
# df['query_len'] = df['url'].apply(lambda x: len(urlparse(x).query))
#
#
# fragment = df['url'].apply(lambda x: urlparse(x).fragment)
# le = LabelEncoder()
# df['fragment'] = le.fit_transform(fragment)
# df['fragment_len'] = df['url'].apply(lambda x: len(urlparse(x).fragment))
#
#
# print('-------------------提取url的各项解析统计特征 end...------------------------')
...
sub.to_csv('main_body_url_nums.csv', index=False)
